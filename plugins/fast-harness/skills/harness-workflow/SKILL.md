...
cross-review. Matt's requirements for TDD, testing seams, ticket shape, and
two-axis code review still apply. If a required Matt skill is unavailable, stop
...
5. For each ticket, record `ticket_base`, follow `implement`, and apply
   `checkpoint-commits` after every coherent verified slice. Supply `ticket_base`
   to the `code-review` phase built into `implement`; do not duplicate that
   review when it completed successfully. Fix accepted findings, rerun the
   relevant checks, checkpoint-commit, ordinary-push, and update ticket status.
6. When all tickets are done, run the full relevant project verification and
   follow `code-review` once more against `workflow_base`. Fix, verify, commit,
   and push any aggregate findings.
7. Run mandatory `cross-review` against `workflow_base`. Verify every finding;
...
2. Record `workflow_base`, follow `implement` directly, and apply
   `checkpoint-commits` after every coherent verified slice. Supply
   `workflow_base` to the `code-review` phase built into `implement`; run the
   phase separately only if `implement` did not complete it.
3. Run relevant project verification. Fix accepted code-review findings,
   verify, commit, and ordinary-push.
4. Run mandatory `cross-review` against `workflow_base`. Verify findings; after
...
Normal `code-review` and different-model `cross-review` are both required; one
does not replace the other. If no peer is available, do not fake or silently
skip cross-review: preserve all commits, finish safe verification, and report
the workflow as blocked on that gate.
...
the route, spec/tickets used, commits and remote branch, checks, both review
results, any preserved pre-existing changes, and any unresolved blocker.
