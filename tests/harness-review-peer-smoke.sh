...
grep -q '^selected_peer_kind=cc$' "$tmp/logs-cc/meta.txt"
grep -q '^peer_transport=external-cli$' "$tmp/logs-cc/meta.txt"
grep -q "^selected_peer_path=$tmp/bin/cc$" "$tmp/logs-cc/meta.txt"
