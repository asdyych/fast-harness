...
明确目标
  ...
  -> checkpoint commit/push
  -> Web 用户流程使用 Trace Browser E2E 校验
  -> cross-review
  -> 相关验证 + 最终 commit/push
```
...
Quick 改动只要影响用户可见的 Web 流程，就必须通过 `e2e-browser` 跑一条代表性的真实浏览器成功路径，并同时检查控制台错误和失败请求；非浏览器可达的 CLI、后端或文档改动会明确记录 E2E 不适用，不做无意义的浏览器校验。
